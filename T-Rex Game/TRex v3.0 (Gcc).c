...
#include<stdlib.h>
#include<conio.h>
int x=20,y=31,cn=0,flag=0,high=0,score=0,dl=80,col=0,cc=0,r=0,f=0,ob[10],sz[10],v=-1,d;
...
void check()
{
    ...
    while(i!=r)
    {
	//if(((x-1==ob[i] || x-2==ob[i] || x-3==ob[i])  && y==31) || ((x-2==ob[i] || x-3==ob[i] || x-4==ob[i]) && y==30))   col=1;
	for(k=1;k<=4;k++)
	{
		for(j=0;j<sz[i];j++)
		{
...
int main()
{
    ...
    if(high<score)
    {high=score;}
    x=20,y=31,cn=0,flag=0,dl=80,score=0,col=0,cc=0,r=0,f=0,v=-1;
